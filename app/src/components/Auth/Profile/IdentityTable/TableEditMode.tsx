...
export const TableEditMode = (props: TableEditMode) => {
    ...
    const [{ firstName, lastName, age, email, password, confirmPassword }, setState] = useState({
        ...
        age: user.age, email: user.email, password: user.password, confirmPassword: user.password
    })

    const _onClickUpdate = () => {
        props.onUpdate({ ...user, first_name: firstName, last_name: lastName, age: age, email: email, password: password })
    }
    const isFormValid = () => {
        return (
            isChange &&
            ...
            password && !validator.isEmpty(password) && password === confirmPassword &&
            email && validator.isEmail(email) &&
            age && validator.isNumeric(String(age)));
    }
    ...
    return (<tbody>
        ...
        <br></br>
        <tr>
            <button type="submit" disabled={!isFormValid()} onClick={_onClickUpdate} className="btn btn-primary btn-block">Save</button>
        </tr>
    </tbody>);
}
